...
double hdd_r[rec_s] =  {3.10, 3.31, 3.52, 3.73, 3.94, 4.14, 4.35, 4.56, 4.77, 4.98, 5.18, 5.39, 5.60, 5.81, 6.02, 6.16}; 
double ssd_r[rec_s] =  {0.53, 1.06, 1.59, 2.12, 2.65, 3.18, 3.71, 4.24, 4.77, 5.29, 5.83, 6.36, 6.89, 7.42, 7.95, 8.48};
double hdd_w[rec_s] =  {3.10, 3.31, 3.52, 3.73, 3.94, 4.14, 4.35, 4.56, 4.77,  4.98,  5.18,  5.39,  5.60,  5.81,  6.02,  6.22};
double ssd_w[rec_s] =  {1.23, 2.46, 3.38, 3.69, 4.92, 6.15, 7.38, 8.61, 9.84, 11.07, 12.30, 13.53, 14.76, 15.99, 17.22, 19.89};
int rCnt[rec_s]; // read hit
int nrCnt[rec_s]; // read miss
int wCnt[rec_s]; // write hit
int nwCnt[rec_s]; // write miss
...
void getssdhddres(double sres,double hres){
	ssdres = sres;
	hddres = hres;

	printf("ssd response time = %f\n",ssdres);
    printf("hdd response time = %f\n",hddres);
}



void record_req( ioreq_event *tmp ) { 
	if(ssdres>50){
	int j = (int) (tmp->bcount+1)/rec_unit; 
	int k = 0;
	int hit = 0;
	if (tmp->flags == 1) {// READ
		for ( k = tmp->blkno; k < tmp->blkno+tmp->bcount; k+=8 ) {
			if (CACHE_SEARCH(fcl_cache_mgr, k)) {
				hit = 1;
				break;
			} else { 
				hit = 0; 
	...
	cnt++;
	if (cnt == 100) { // period, decide seq_unit	
		double hdd_cost[rec_s];
		double ssd_cost[rec_s];
		...
		for ( i = 0; i < rec_s; i++ ) {
			...
			double r_gamma = 0.0;
			if ((rCnt[i]+nrCnt[i])!=0)
				r_gamma = r_victim[i]/(rCnt[i]+nrCnt[i]);

			double w_gamma = 0.0;
			if ((wCnt[i]+nwCnt[i])!=0)
				w_gamma = w_victim[i]/(wCnt[i]+nwCnt[i]);
			hdd_cost[i] = 0.0;
			ssd_cost[i] = 0.0;
			
			prCnt[i] = (1-beta)*prCnt[i]+beta*rCnt[i];
			pnrCnt[i] = (1-beta)*pnrCnt[i]+beta*nrCnt[i];
			pwCnt[i] = (1-beta)*pwCnt[i]+beta*wCnt[i];
			pnwCnt[i] = (1-beta)*pnwCnt[i]+beta*nwCnt[i];

			// read hit: SSD, write: HDD, read miss: HDD			
			hdd_cost[i] = (prCnt[i]*ssd_r[i]) 
					+ ((pwCnt[i]+pnwCnt[i])*hdd_w[i])
					+ (pnrCnt[i]*hdd_r[i]);
		
			// read hit: SSD, read miss: stage(HDDr+SSDw) + reclaim(SSDr+HDDw)
			// write hit: SSD, write miss: SSD + reclaim(SSDr+HDDw)
			ssd_cost[i] = (prCnt[i]*ssd_r[i])
					+ (pwCnt[i]*ssd_w[i]) 
					+ (pnwCnt[i]*(ssd_w[i]+(ssd_r[i]+hdd_w[i])*w_gamma) )
					+ (pnrCnt[i]*(hdd_r[i]+ssd_w[i]+(ssd_r[i]+hdd_w[i])*r_gamma)); 
			


		} // end for
		int l = 0, r = rec_s-1;
		double rCost = 0.0, lCost = 0.0;		
		while ( l != r ) {		
			lCost += ssd_cost[l];
			ssd_cost[l] = 0.0;
			rCost += hdd_cost[r];
		...
		fcl_params->fpa_seq_unit_size = r;
		reset_record( 1 );

		}
		if(ssdres > hddres){
			struct lru_node *ln;
   			ln = CACHE_SEARCH(fcl_cache_mgr, tmp->blkno);
   			if( tmp->devno == SSD && ln->cn_dirty == 0){
   			set_sequnit(-10000000); 
																	...
																	ioreq_event *child )
{
	...
	if ( last == NULL ) {
		...
		if (!(child->devno == last->devno && child->flags == last->flags)) {
			printf(" list index = %d, cdev = %d, cflags = %d, ldev = %d lflags = %d \n", list_index,
					child->devno, child->flags, last->devno, last->flags);
		} 
